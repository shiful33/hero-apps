...
import installationApp from '../assets/installationApp.png';
...
const Installation = () => {
    ...
    const handleUninstall = (appId, appTitle) => {
        ...
        toast.success(`${appTitle} Uninstalled From Your Device!`, {
            duration: 3000,
            style: {
    ...
    return (
        <div className='p-[20px] lg:p-[80px]'>
            <h1 className='gradient-text text-[28px] md:text-[38px] lg:text-[48px] font-bold mb-[16px] text-center flex justify-center items-center gap-4'>Your Installed Apps <img src={installationApp} className='w-[80px]' alt="" /></h1>
            ...
            {installedApps.length === 0 ? (
                ...
                <div className='flex flex-col gap-5'>
                 {
                    sortedApp.map(app => (
                        <div key={app.id} className='flex items-center justify-between p-5 bg-white rounded-lg shadow-sm'>
                        ...
                        <button onClick={() => handleUninstall(app.id, app.title)} className='bg-[#00d390] text-white py-2 px-4 rounded-md hover:bg-[#00b57a]'>Uninstall</button>
                        </div>
                    ))}
                </div>
            )}
